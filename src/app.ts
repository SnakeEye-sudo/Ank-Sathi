...
(() => {
  ...
  const saved = JSON.parse(localStorage.getItem(STORAGE_KEY) || "{}") as { name?: string; dob?: string };
  ...
  const meanings: Record<number, { title: string; energy: string; strength: string; caution: string }> = {
  ...
  const monthThemes: Record<number, { theme: string; advice: string }> = {
  ...
  function letterValue(char: string) {
  ...
  function nameSum(name: string, picker: (char: string) => boolean) {
  ...
  function numberMeaning(value: number) {
    return meanings[value] || meanings[reduceNumber(value)] || meanings[1];
  }

  function personalYear(month: number, day: number, year: number) {
  ...
  function calculateReading(name: string, dob: string): Reading | null {
    if (!dob) return null;
  ...
  function buildForecast(dob: string, reading: Reading): MonthForecast[] {
  ...
  function renderReading(name: string, dob: string) {
